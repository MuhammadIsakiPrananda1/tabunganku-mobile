<p align="center">
  <a target="_blank" rel="noopener noreferrer nofollow" href="https://raw.githubusercontent.com/MuhammadIsakiPrananda1/tabunganku-mobile/main/assets/promo_banner.png"><img src="https://raw.githubusercontent.com/MuhammadIsakiPrananda1/tabunganku-mobile/main/assets/promo_banner.png" alt="TabunganKu Banner" width="100%" style="max-width: 100%;"></a>
</p>
<p align="center">
  <a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/Versi-1.5.2-blue?style=for-the-badge"><img src="https://img.shields.io/badge/Versi-1.5.2-blue?style=for-the-badge" alt="Versi" style="max-width: 100%;"></a>
  <a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/Flutter-3.0.0+-02569B?style=for-the-badge&logo=flutter&logoColor=white"><img src="https://img.shields.io/badge/Flutter-3.0.0+-02569B?style=for-the-badge&logo=flutter&logoColor=white" alt="Flutter" style="max-width: 100%;"></a>
  <a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/State-Riverpod-764ABC?style=for-the-badge&logo=riverpod&logoColor=white"><img src="https://img.shields.io/badge/State-Riverpod-764ABC?style=for-the-badge&logo=riverpod&logoColor=white" alt="Riverpod" style="max-width: 100%;"></a>
  <a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/Backend-Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black"><img src="https://img.shields.io/badge/Backend-Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black" alt="Backend" style="max-width: 100%;"></a>
  <a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/Lisensi-MIT-green?style=for-the-badge"><img src="https://img.shields.io/badge/Lisensi-MIT-green?style=for-the-badge" alt="Lisensi" style="max-width: 100%;"></a>
</p>

---

# 💎 TabunganKu Versi 1.5.2

Rilis v1.5.2 membawa lompatan besar dalam perencanaan keuangan mandiri, optimalisasi performa tinggi, dan pengalaman antarmuka visual yang memukau. Kami memperkenalkan modul Brankas Payday, Alokasi THR & Bonus, Split Bill & Patungan, Anggaran Belanja, Simulasi Pensiun Dini (FIRE Plus), Time Value of Money (TVM), serta evaluasi kesehatan finansial terpadu.

---

### 🆕 Fitur Yang Ditambah

#### 🏦 Brankas Payday (Payday Vault)
Alokasi gaji otomatis terpusat saat tanggal gajian tiba dengan fitur penguncian saldo pos darurat, investasi, dan tabungan terencana agar terhindar dari pemborosan awal bulan.

#### 🎁 Alokasi THR & Bonus (THR Bonus Planner)
Modul cerdas untuk membagi dan mengalokasikan Tunjangan Hari Raya (THR) dan bonus kerja ke pos prioritas secara terstruktur dan proporsional.

#### 🍽️ Split Bill & Patungan
Kalkulator patungan makan dan belanja bersama yang fleksibel dengan pembagian otomatis pajak restoran, biaya servis, dan diskon antar anggota secara adil dan transparan.

#### 🛒 Anggaran Belanja (Shopping Budget Planner)
Rencana belanja kebutuhan harian dan mingguan dengan batas pagu anggaran serta pelacakan realisasi pengeluaran dan status sisa anggaran.

#### 🌅 Simulasi Pensiun Dini (Early Retirement / FIRE Plus)
Kalkulator pensiun dini komprehensif yang memperhitungkan rasio tabungan, proyeksi inflasi, target usia, dan perkiraan akumulasi dana FIRE yang dibutuhkan.

#### 📈 Time Value of Money (TVM) Calculator
Modul edukasi dan perhitungan nilai waktu uang (Present Value ke Future Value dan sebaliknya) untuk membantu analisis investasi jangka panjang.

#### 🩺 Financial Health Checkup Terpadu
Evaluasi komprehensif kesehatan finansial pribadi dengan metrik rasio likuiditas, rasio utang terhadap aset, rasio tabungan bulanan, dan skor kesehatan finansial keseluruhan.

#### 💬 Kritik & Saran (Feedback Page)
Halaman umpan balik terintegrasi bagi pengguna untuk mengirim saran fitur, laporan bug, atau apresiasi langsung dari dalam aplikasi.

#### 🎨 Tipografi & Visual Baru
*   Integrasi font modern Google Inter dan Plus Jakarta Sans.
*   Widget ilustrasi ikon vektor kartun ekspresif.
*   Komponen teks berjalan (*Marquee*) untuk informasi dinamis.
*   Dialog loading offline dan dialog penyesuaian tabungan yang lebih interaktif.

---

### 🛠️ Fitur Yang Diubah

#### 📊 Dashboard & Tab Navigation Overhaul
*   Optimalisasi rendering kartu dashboard dan grafik keuangan.
*   Penyempurnaan fitur sembunyikan/tampilkan saldo (*Balance Visibility*).
*   Tata letak tab navigasi yang lebih responsif dan halus.

#### 🏷️ Transaction Categories Optimization
*   Pembaruan menyeluruh struktur kategori transaksi dengan klasifikasi mendalam untuk kebutuhan keluarga, pendidikan, dan bisnis.

#### ⚙️ Settings & Security Enhancements
*   Penguatan alur pengaturan PIN keamanan dan preferensi tampilan.
*   Penyelarasan badge pencapaian (*Achievement Provider*) dan layanan notifikasi.

#### 🎭 Theme & Color System Refinement
*   Penyesuaian skema warna yang lebih harmonis pada seluruh komponen tombol, bottom sheet, dan dialog.

---

### 🗑️ Fitur Yang Dihapus

#### 🚫 Modul Legacy & Pembersihan Aset
*   Penghapusan halaman lama yang telah digantikan oleh modul baru (`financial_health_page.dart`, `lifestyle_saving_page.dart`, `qris_services_page.dart`).
*   Pembersihan file gambar QRIS lama (`qris_kelontong_zaky.jpg`, `qris_zaky_store.jpg`).

---

### ⚡ Detail Teknis & Performa
*   **Build**: v1.5.2-stable (Build 152)
*   **Engine**: Flutter 3.47.2 (Channel Stable)
*   **State Management**: Flutter Riverpod 2.x
*   **OS Support**: Android 6.0+ (Marshmallow)
*   **Release Assets**: Split APK (arm64-v8a, armeabi-v7a, x86_64)

---

Terima kasih telah menemani perjalanan evolusi TabunganKu hingga versi 1.5.2. Kami berkomitmen untuk selalu mendampingi perencanaan finansial Anda dengan teknologi cerdas dan estetika premium.

**Kendali Penuh Keuangan, Masa Depan Cemerlang!** 💰✨
